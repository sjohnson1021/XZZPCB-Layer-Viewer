...
PCBViewerWindow::PCBViewerWindow(
    ...
    : m_camera(camera), m_viewport(viewport), m_grid(grid), m_gridSettings(gridSettings), m_controlSettings(controlSettings), m_boardDataManager(boardDataManager), m_isOpen(true), m_isFocused(false), m_isHovered(false), m_isContentRegionHovered(false), m_renderTexture(nullptr), m_textureWidth(100), m_textureHeight(100), m_contentRegionTopLeftScreen(ImVec2(0.0f, 0.0f)), m_contentRegionSize(ImVec2(100.0f, 100.0f)), m_desiredTextureSize(ImVec2(0.0f, 0.0f)), m_resizeCooldownFrames(-1)
{
...
void PCBViewerWindow::RenderIntegrated(SDL_Renderer *sdlRenderer, PcbRenderer *pcbRenderer, const std::function<void()> &pcbRenderCallback)
{
    ...
    if (window_open_this_frame)
    {
        ...
        if (pcbRenderer && pcbRenderer->WasFrameJustRendered())
        {
        ...
        if (m_renderTexture)
        {
            ...
            if (m_interactionManager && (m_isFocused || m_isHovered))
            {
                ...
                m_interactionManager->ProcessInput(ImGui::GetIO(), m_isFocused, m_isContentRegionHovered, m_contentRegionTopLeftScreen, m_contentRegionSize, pcbRenderer);
            }
            else
            {
                m_isContentRegionHovered = false;
    ...
    else
    {
        ...
        m_isContentRegionHovered = false;
        if (m_renderTexture)
        { // If window is effectively closed by ImGui, release texture
            ReleaseTexture();
        }
    }
...
void PCBViewerWindow::OnBoardLoaded(const std::shared_ptr<Board> &board, PcbRenderer *pcbRenderer)
{
    if (board)
    {
    ...
    if (pcbRenderer)
    {
        ...
        pcbRenderer->MarkGridDirty(); // Also mark grid dirty as board extents might change grid appearance
    }
}