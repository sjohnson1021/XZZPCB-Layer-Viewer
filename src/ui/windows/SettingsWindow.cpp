...
SettingsWindow::SettingsWindow(std::shared_ptr<GridSettings> gridSettings,
    ...
    : m_gridSettings(gridSettings), m_controlSettings(controlSettings), m_boardDataManager(boardDataManager), m_appClearColor(applicationClearColor) // Store the pointer
      ,
...
void SettingsWindow::ShowGridSettings()
{
    ...
    if (ImGui::CollapsingHeader("Grid Options", ImGuiTreeNodeFlags_DefaultOpen))
    {
        ...
        if (m_gridSettings->m_visible)
        {
            ...
            // Display unit
            const char *unitStr = m_gridSettings->UnitToString();
            ...
            if (m_gridSettings->m_isDynamic)
            {
                ...
                if (m_gridSettings->m_minPixelStep != pixelStepValues[minPixelStepIndex])
                {
                ...
                if (m_gridSettings->m_maxPixelStep != pixelStepValues[maxPixelStepIndex])
                {
                    for (int i = 0; i < numOptions; i++)
                    {
            ...
            if (m_gridSettings->m_unitSystem == GridUnitSystem::METRIC)
            {
                // For metric, allow 1mm to 1000mm (1m) major spacing
                ImGui::DragFloat(("Major Spacing (" + std::string(unitStr) + ")").c_str(),
                                 &m_gridSettings->m_baseMajorSpacing, 1.0f, 1.0f, 1000.0f, "%.2f");
                if (ImGui::IsItemHovered())
                {
            ...
            else
            {
                ...
                ImGui::DragFloat(("Major Spacing (" + std::string(unitStr) + ")").c_str(),
                                 &m_gridSettings->m_baseMajorSpacing, 0.125f, 0.1f, 36.0f, "%.3f");
                if (ImGui::IsItemHovered())
                {
            ...
            ImGui::Checkbox("Show Measurement Readout", &m_gridSettings->m_showMeasurementReadout);
            if (ImGui::IsItemHovered())
            {
            ...
            ImGui::TextWrapped("Maximum renderable grid lines: %d", m_gridSettings->MAX_RENDERABLE_LINES);
            ImGui::TextWrapped("Maximum renderable grid dots: %d", m_gridSettings->MAX_RENDERABLE_DOTS);
...
namespace
{
    ...
    KeyCombination CaptureKeybind()
    {
        ...
        for (ImGuiKey key = ImGuiKey_NamedKey_BEGIN; key < ImGuiKey_NamedKey_END; key = (ImGuiKey)(key + 1))
        {
            ...
            { // `false` for `ImGuiKeyOwner_Any`, check if key was pressed this frame
                newKeybind.key = key;
                break;
            }
        }
        ...
        if (newKeybind.key != ImGuiKey_None)
        { // Only set modifiers if a main key was pressed
    ...
    if (ImGui::CollapsingHeader("Application Keybinds", ImGuiTreeNodeFlags_None))
    {
        ...
        if (ImGui::BeginTable("keybindsTable", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg))
        {
            ...
            for (int i = 0; i < static_cast<int>(InputAction::Count); ++i)
            {
                ...
                if (ImGui::Button(buttonLabel.c_str(), ImVec2(-FLT_MIN, 0)))
                { // -FLT_MIN makes button stretch
                    if (actionToRebind == currentAction)
                    { // Was capturing, now cancel
