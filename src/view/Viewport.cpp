...
Viewport::Viewport() : m_screenX(0), m_screenY(0), m_screenWidth(0), m_screenHeight(0) {}

Viewport::Viewport(int screenX, int screenY, int screenWidth, int screenHeight) : m_screenX(screenX), m_screenY(screenY), m_screenWidth(screenWidth), m_screenHeight(screenHeight) {}
...
Vec2 Viewport::ScreenToWorld(const Vec2& screenPoint, const Camera& camera) const
{
    ...
    Vec2 pointInViewport = {
        screenPoint.x_ax - (m_screenX + m_screenWidth / 2.0f),
        screenPoint.y_ax - (m_screenY + m_screenHeight / 2.0f)  // Y is now down, centered
    };
    ...
    Vec2 pointInCameraSpace = {pointInCameraSpaceNoRotation.x_ax * cosA - pointInCameraSpaceNoRotation.y_ax * sinA,
                               pointInCameraSpaceNoRotation.x_ax * sinA + pointInCameraSpaceNoRotation.y_ax * cosA};
...
Vec2 Viewport::WorldToScreen(const Vec2& worldPoint, const Camera& camera) const
{
    ...
    Vec2 pointInCameraAxes = {pointRelativeToCamera.x_ax * cosA + pointRelativeToCamera.y_ax * sinA, -pointRelativeToCamera.x_ax * sinA + pointRelativeToCamera.y_ax * cosA};
    ...
    Vec2 screenPoint = {
        (m_screenX + m_screenWidth / 2.0f) + pointInViewSpace.x_ax,
        (m_screenY + m_screenHeight / 2.0f) + pointInViewSpace.y_ax  // Y is now down
    };
...
Vec2 Viewport::ScreenDeltaToWorldDelta(const Vec2& screenDelta, const Camera& camera) const
{
    ...
    Vec2 adjustedScreenDelta = {screenDelta.x_ax, screenDelta.y_ax};
    ...
    Vec2 worldDelta = {worldDeltaNoRotation.x_ax * cosA - worldDeltaNoRotation.y_ax * sinA, worldDeltaNoRotation.x_ax * sinA + worldDeltaNoRotation.y_ax * cosA};
...
Vec2 Viewport::WorldDeltaToScreenDelta(const Vec2& worldDelta, const Camera& camera) const
{
    ...
    Vec2 screenDeltaNoZoom = {// R(-A) * worldDelta
                              worldDelta.x_ax * cosA + worldDelta.y_ax * sinA,
                              -worldDelta.x_ax * sinA + worldDelta.y_ax * cosA};

    // Scale by zoom.
    Vec2 screenDeltaYDown = screenDeltaNoZoom * camera.GetZoom();

    // Y-axis is already screen Y-down.
    return Vec2(screenDeltaYDown.x_ax, screenDeltaYDown.y_ax);
}
